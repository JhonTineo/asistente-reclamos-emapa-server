...
class ObjetivosAgent:
    ...
    def generar(self, motivo: str, clasificacion: str = "") -> list[ObjetivoInvestigacion]:
        ...
        system = (
            ...
            "sí solo, haría FUNDADO este reclamo?"
            ...
            "- Distingue SÍNTOMA de CAUSA: un objetivo que solo CONFIRMA el síntoma que "
            "reclama el cliente (p. ej. «¿el consumo del mes es más alto que el promedio?») "
            "NO es determinante por sí solo — confirmar que el consumo es alto no resuelve "
            "el reclamo, solo indica que hay algo que explicar. Son determinantes los "
            "objetivos que establecen la CAUSA o la legitimidad del cobro (inspección de "
            "fugas o del medidor, método/historial de facturación, error de medición): cada "
            "uno puede, por sí solo, inclinar el veredicto.\n"
            ...
            "decide por sí solo\"}]\n\n"
            "Ejemplo (reclamo por consumo medido alto; el cliente NO menciona fuga, solo "
            "que su consumo habitual es menor):\n"
            "[{\"descripcion\": \"¿El consumo del mes reclamado es realmente mayor que el "
            "promedio histórico del predio?\", \"medio\": \"tarjeta_lectura\", \"determinante\": "
            "false, \"porque_determinante\": \"solo confirma el síntoma; que el consumo sea "
            "alto no resuelve el reclamo por sí solo\"}, {\"descripcion\": \"¿La facturación "
            "previa venía por promedio y pasó a medido, generando el desajuste?\", \"medio\": "
            "\"record_facturacion\", \"determinante\": true, \"porque_determinante\": \"si el "
            "alza se explica por el paso de promedio a medido, el cobro es correcto "
            "(INFUNDADO)\"}, {\"descripcion\": \"¿La inspección halló una fuga (visible o no "
            "visible) que explique el consumo?\", \"medio\": \"inspeccion_interna\", "
            "\"determinante\": true, \"porque_determinante\": \"una fuga no visible reparada "
            "obliga a refacturar por promedio (FUNDADO); una fuga visible se factura por "
            "diferencia (INFUNDADO)\"}]\n\n"
